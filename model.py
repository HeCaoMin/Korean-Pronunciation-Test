...
class VladPooling(keras.engine.Layer):
    '''
    This layer follows the NetVlad, GhostVlad  该层遵循 NetVlad、GhostVlad
    '''
...
def vggvox_resnet2d_icassp(input_dim=(257, 250, 1), num_class=8631, mode='train', args=None):
    ...
    # ===============================================
    if loss == 'softmax':
        y = keras.layers.Dense(num_class, activation='softmax', #神经网络的最后一层激活函数为softmax
                               kernel_initializer='orthogonal',
                               use_bias=False, trainable=True,
                               kernel_regularizer=keras.regularizers.l2(weight_decay),
                               bias_regularizer=keras.regularizers.l2(weight_decay),
                               name='prediction')(x)
        trnloss = 'categorical_crossentropy'# 交叉熵损失函数
    ...
    if mode == 'train':
        ...
        # set up optimizer.
        if args.optimizer == 'adam':  opt = keras.optimizers.Adam(lr=1e-3)# 优化器 adam 学习率设置0.001
